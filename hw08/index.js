...
function setupKeyboardEvents() {
  document.addEventListener("keydown", (event) => {
    if (event.key == "a") {
      ...
      updateText(
        textOverlay3,
        `shading mode: ${shadingMode} (${renderingMode})`,
      );
      render();
    } else if (event.key == "f") {
...
function render() {
  ...
  if (renderingMode == "GOURAUD") {
    gouraudShader.use(); // using the cone's shader (gouraud)
    ...
    // renderingMode == "PHONG"
    phongShader.use(); // using the cone's shader (phong)
...
async function main() {
  try {
    ...
    [gouraudShader, phongShader].forEach((shader) => {
      shader.use();
